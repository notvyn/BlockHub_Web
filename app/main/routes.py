...
# Deals only with dynamic user data
@main.app_context_processor
def inject_global_badges():
    ...
    if current_user.is_authenticated:
        ...
        # Get the list of IDs the user has already read
        ...
        # Count the announcements that are NOT in that list
        if read_ids:
...
def dashboard():
    ...
    if current_user.is_authenticated:
        ...
        # Fetch up to 3 UNREAD announcements (Newest first)
        ...
        # If there are less than 3 unread, fill the gap with the newest READ announcements
        ...
        # Sort the final combined list of 3 chronologically 
    ...
    # DEADLINES & OTHER DATA

    # Grab the exact UTC time, remove timezone info, and add 8 hours for PHT
    now_utc = datetime.now(timezone.utc).replace(tzinfo=None)
    ph_time = now_utc + timedelta(hours=8)
    
    # Calculate the 24-hour grace period based on Philippine Time
    cutoff_time = ph_time - timedelta(days=1)

    completed_ids = []

    if current_user.is_authenticated:
    # What has the user already checked off?
        completed_ids = [c.deadline_id for c in DeadlineCompletion.query.filter_by(user_id=current_user.id).all()]

    # Start the query: Only get tasks where the deadline is STILL IN THE FUTURE (or grace period)
    active_query = Deadline.query.filter(Deadline.due_date >= cutoff_time)
    
    # Filter out the ones they already clicked "Done" on
    if completed_ids:
    ...
    # For Strict week count
    # Math: Sunday is 6. If today is Wed (2), 6 - 2 = 4 days until Sunday.
    # days_until_sunday = 6 - ph_time.weekday()
    
    # Add those days to today's date to find the exact date of this Sunday
    # end_of_week = ph_time + timedelta(days=days_until_sunday)

    # Grab the absolute newest record, regardless of time.
    target_record = ClassSummary.query.order_by(ClassSummary.date_held.desc()).first()
    
    # Create an empty dictionary to hold our grouped data
    daily_summaries = {}
    
    if target_record:
        # Extract just the calendar date from the newest record
        ...
        # If it is 3 days old or less, fetch all summaries for that calendar day
        if days_old <= 3:
            # Fetch the raw summaries just like before
...
def announcement(id):
    ...
    # --- BACKEND READ RECEIPT SAFETY NET ---
    ...
    # Fetch the CURRENT USER'S specific interactions
    # Define this variable outside the if-statement so logged-out users don't crash the page
    user_hearted = []
    
    if current_user.is_authenticated:
        # Filter by BOTH the user's ID and the announcement ID
        heart_record = AnnouncementHeart.query.filter_by(
            user_id=current_user.id, 
            announcement_id=id
        ).first()
        
        # If a record exists, append the ID so the HTML {% if announcement.id in user_hearted %} works
        if heart_record:
...
def announcements():
    ...
    # Prepare empty containers for our frontend data
    heart_counts = {}
    user_hearted_ids = []
    read_stats = {}
    read_ids = [] # Container for the logged-in user's read receipts
    
    # Process data for EACH announcement
...
@login_required
def blockmates():
    # Fetch all users, sort alphabetically by name
...
def courses():
    ...
    # Setup a dictionary to hold schedules grouped by the days of the week
    days_order = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
    master_schedule = {day: [] for day in days_order}
    
    # Fetch ALL schedules and join them with their Course data
...
def notifications():
    ...
    # Fetch the 20 newest Announcements
    ...
    #  Fetch the 20 newest Class Summaries
    ...
    # The Sorting Fix (Date vs. Datetime)
    ...
    # Sort the master list (Newest items at the very top)
    master_feed.sort(key=lambda x: x['sortable_date'], reverse=True)

    # Pass the final, sorted feed to your HTML
    return render_template(
        'notifications.html', 
...
@login_required
def summary(id):
    # Fetch ALL summaries, ordering by newest date first
...
@login_required
def summaries():
    # Get the requested year and week from the URL. 
    ...
    # Calculate the exact calendar dates for Monday and Sunday of this specific week
    ...
    # Fetch ONLY the summaries that fall between this Monday and Sunday
    ...
    #  Your exact existing logic for grouping them by day goes here!
    grouped_summaries = {}
    for summary in raw_summaries:
    ...
    # Calculate the dates for the "Previous" and "Next" buttons
...
def add_announcement():
    ...
    if form.validate_on_submit():
        ...
        for sub in all_subscriptions:
            # Use the helper method in models.py to turn the text back into a dictionary
...
def add_summary():
    ...
    # Populate ALL schedules so WTForms validation passes on POST
...
def update_profile():
    ...
    # Fetch all tags and store them in a variable
    all_tags = Tag.query.all()
    
    # Use that variable for your choices
    ...
    if form.validate_on_submit():
        ...
        # PRE-CHECK THE BOXES THEY ALREADY OWN
