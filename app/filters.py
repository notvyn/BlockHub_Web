...
def markdown_filter(text):
    ...
    # The VIP List: Exactly which HTML tags are allowed to exist
    allowed_tags = [
    ...
    # Allowed Attributes: Prevent malicious links or image sources
    allowed_attrs = {
        'a': ['href', 'title', 'target'],
        'img': ['src', 'alt', 'title']
    }
    
    # Scrub the HTML clean with Bleach
    clean_html = bleach.clean(raw_html, tags=allowed_tags, attributes=allowed_attrs)
    
    # Markup() tells Jinja the code is now perfectly safe to render
...
def parse_links_filter(text):
    ...
    for title, raw_url in markdown_links:
        ...
        # If you typed a Markdown link without https://, add it automatically
        if not clean_url.startswith(('http://', 'https://')):
            clean_url = f"https://{clean_url}"
            
        formatted_links.append((title, clean_url))
        
    # --- Prepare Text for Raw Links ---
    # ERASE the markdown links from the text block before running the raw URL search.
    ...
    for url in raw_urls:
        ...
        # If it's a naked domain like 'www.instagram.com', add https://
        if not clean_url.startswith(('http://', 'https://')):
            clean_url = f"https://{clean_url}"
